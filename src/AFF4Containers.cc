	...
	namespace container {
		...
		std::string getResourceID(const std::string& filename) NOEXCEPT {
			...
			*/
			...
			std::string resource(data, (uintptr_t)segmentEntrySize);
			return resource;
			}
			/*
...
slowPath:
			...
			if (stream != nullptr) {
				std::unique_ptr<char[]> buffer(new char[(uintptr_t)stream->size()]);
				int64_t res = stream->read(buffer.get(), stream->size(), 0);
				if (res > 0) {
					resourceSP = std::string(buffer.get(), (uintptr_t)res);
				}
				stream->close();
			}
			zipFile->close();
